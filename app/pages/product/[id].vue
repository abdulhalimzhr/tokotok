<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
    ...
    <div v-else-if="error" class="max-w-5xl mx-auto px-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 text-center">
        <div class="text-red-500 dark:text-red-400 mb-4">
          <Icon name="i-heroicons-exclamation-triangle" class="w-16 h-16 mx-auto mb-2" />
    ...
    <div v-else-if="product">
      <nav class="max-w-5xl mx-auto mb-4 px-4" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          <li>
            <NuxtLink to="/" class="hover:underline hover:text-green-600 dark:hover:text-green-400">Home</NuxtLink>
          </li>
          <li>
            <span class="mx-1">/</span>
          </li>
          <li>
            <NuxtLink to="/" class="hover:underline hover:text-green-600 dark:hover:text-green-400">Products</NuxtLink>
          </li>
          <li>
            <span class="mx-1">/</span>
          </li>
          <li class="truncate max-w-xs" :title="product.title">
            <span class="text-gray-700 dark:text-gray-200">{{ product.title }}</span>
          </li>
        </ol>
      </nav>
      
      <div class="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow p-6 flex flex-col md:flex-row gap-8">
        ...
        <div class="flex-1 flex flex-col justify-between">
          <div>
            ...
            <div class="flex items-center space-x-2 mb-2">
              <span class="text-green-600 dark:text-green-400 text-xs font-semibold uppercase">
                {{ product.category }}
              </span>
          ...
          <div class="mt-6">
            <h2 class="text-lg font-semibold mb-2 dark:text-gray-100">Description</h2>
...
<script setup>
  ...
  useSeoMeta({
    title: () => product.value?.title ? `${product.value.title} - TokoTok` : 'Product - TokoTok',
    description: () => product.value?.description || 'Product details from TokoTok store'
  });
